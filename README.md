...
This file contains a utility to sanitize and normalize product names. It removes special characters, brackets, and collapses multiple spaces, converting everything to lowercase. This is essential to ensure consistent, exact string matching between different data sources (AI-generated names, actual DB products, and frontend inputs) so that duplications are avoided.
...
**Why the code is there & What it is doing:**
This is the Mongoose schema defining the `geo_catalogs` collection. It structures the data to support a multi-level geographic hierarchy (`PINCODE`, `CITY`, `STATE`, `COUNTRY`). Crucially, it organizes products within each category into two distinct sections: `trending` (sourced from our internal database frequency) and `popular` (generated by the AI to fill inventory gaps).
...
**Why the code is there & What it is doing:**
This service handles the matching of raw generated product names against the master catalogue. It takes large arrays of product names, deduplicates them, and matches them in chunked queries to avoid database timeouts. This ensures that only legitimate, orderable products with valid IDs and images are returned and stored.
...
**Why the code is there & What it is doing:**
This file manages the integration with Google Gemini AI. It builds intelligent, dynamic prompts depending on whether a category is "fresh" (e.g., vegetables, needing generic names) or "packaged" (e.g., snacks, needing brand names). It cleanly parses the JSON response, includes retry logic for insufficient results, and provides extensive error classification for debugging.
...
**Why the code is there & What it is doing:**
This service is responsible for retrieving geographic location data (Pincode, City, State) directly from the PostgreSQL database using Knex. By circumventing slow external HTTP requests or S3 JSON lookups, it heavily optimizes the performance needed for the nightly cron job to group and aggregate verified shops.
...
**Why the code is there & What it is doing:**
This is the core engine that orchestrates the progressive building of catalogs at each geographic level. It retrieves actual shop data, uses the AI service to augment missing items, matches everything against the master catalogue to strip out invalid items, and finally saves the fully merged and categorized document to MongoDB.
...
**Why the code is there & What it is doing:**
This service manages the fallback retrieval logic for the frontend and shop syncing. If a highly specific `PINCODE` catalog is not available, it systematically queries broader regions (`CITY` -> `STATE` -> `COUNTRY`) to guarantee that the frontend always receives a rich, populated catalog.
...
**Why the code is there & What it is doing:**
Interacts with the PostgreSQL master catalog database. It handles fetching active products and categories, serving as the definitive source of truth to validate if AI-generated or loosely-typed products correspond to actual inventory in the SARVM system.
...
**Why the code is there & What it is doing:**
This service calculates and retrieves trending items based on actual transaction history, shop inventories, or search frequencies. The output from this service forms the "trending" (DB-sourced) half of the geographic catalog before the AI supplements it.
...
**Why the code is there & What it is doing:**
This service is responsible for onboarding new, unverified, or imported shops. It automatically assigns them customized catalogs based on the pre-built geographic catalogs of their specific region, drastically reducing cold-start problems for new merchants.
...
**Why the code is there & What it is doing:**
The Express controller handling HTTP requests for popular/trending products. It parses incoming query parameters, maps requests to the `popularProduct.service`, and returns the standardized JSON response to the frontend.
...
**Why the code is there & What it is doing:**
The Express controller exposing specific endpoints to manually interact with the Geo Catalog system. This includes APIs to test the catalog generation for a specific pincode/shopId, trigger the cron job immediately, and fetch the generated fallback catalogs.
...
**Why the code is there & What it is doing:**
Defines the REST API routes for geographic catalog operations (e.g., `/test/pincode`, `/apply`, `/cron/trigger`, `/catalog`) and maps them to the respective methods in the `geo.js` controller.
...
**Why the code is there & What it is doing:**
This is the background cron job scheduled to run nightly at 2:00 AM IST. It is highly fault-tolerant; it iteratively builds the Pincode, City, State, and Country catalogs sequentially with enforced delays to prevent overloading external AI APIs, and gracefully skips any failed regions without crashing the whole process.
...
**Why the code is there & What it is doing:**
The central Express route aggregator. It registers and mounts the new geographic catalog routes (`v1/geo`) alongside the rest of the existing application endpoints, making them available on the server.
...
**Why the code is there & What it is doing:**
The core application startup and initialization module. It establishes database connections, starts the Express server, and, crucially for this feature, initializes the `geoCatalog.job` cron scheduler so it runs automatically in production.
